...
function Login() {
  ...
  const handleChangePassword = () => {
    ...
    axios
      .post(URL.RESET_PASSWORD + email.current?.value)
      .then((res) => {
        console.log(res);
        if (res.data.responseCode == Constants.OK_200) {
          setLoginError(true);
      ...
      .catch((err) => {
        if (err.response && err.response.status == Constants.NOT_FOUND_404) {
          ...
          err.response &&
          err.response.status == Constants.SERVER_ERROR_500
        ) {
          setLoginError(true);
          setcustomSeverity("error");
          setTimeout(() => {
  ...
  const handleGoogleLogin = () => {
    ...
    window.open(URL.GOOGLE_AUTH_URL,'_blank');
    // window.postMessage('Google',URL.GOOGLE_AUTH_URL)
    ...
    <div className="Login">
      ...
      <div className="alertLogin">
        <Collapse in={loginError}>
          <Alert severity={customSeverity} sx={{ mb: 1 }}>
            {message}
          </Alert>
        </Collapse>
      </div>
      ...
      {!changePassword && (
        <>
          ...
          <div>
            <a href="/" onClick={(e) => showChangePassword(e)}>
              Forgot password
            </a>
          </div>
          <LoadingButton
          ...
          <Button onClick={handleGoogleLogin} style={{ color: "black", backgroundColor: "white" }}  variant="outlined" startIcon={<GoogleIcon />}>
            login with google
          </Button>
        </>
      )}
